...
class TodoistApi:
    def __init__(self, todoist_api_token):
        ...
        self.sync_api = Sync_API(todoist_api_token, api_version='v9')
    ...
    def load_all_objects(self):
        ...
        self.done_tasks = scope['done_tasks']
    ...
    def _process_scope_diff(self, scope):
        ...
        new_tasks = scope['tasks'].keys() - self.tasks.keys()
        ...
        completed_tasks = self.done_tasks.keys() - scope['done_tasks']
    ...
    def _send_message_via_bot(text, delete_previous=False, save_msg_to_db=True):
        ...
        request += f'?chat_id={config.alexx_chat_id}'
        request += f'&text={text}'
        if delete_previous:
